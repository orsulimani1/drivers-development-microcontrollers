...
// 5. Configure timing (master mode only) using i2c_configure_timing()
   i2c_status_t status = i2c_configure_timing(i2c, config->clock_speed);
   if (status != I2C_STATUS_OK) {
      return status;
   }

// 6. Configure own address in OAR1 register
   i2c->OAR1.fields.ADD7_1 = 0x00; // Own address = 0x00
   i2c->OAR1.fields.ADDMODE = (config->addr_mode == I2C_ADDR_10BIT) ? 1 : 0;
// 7. Configure CR1 register (ACK, NOSTRETCH)
   i2c->CR1.fields.ACK = (config->ack_enable == I2C_ACK_ENABLE) ? 1 : 0;
   i2c->CR1.fields.NOSTRETCH = (config->no_stretch == I2C_STRETCH_DISABLE) ? 1 : 0;
...
i2c_status_t i2c_deinit(i2c_config_t *config){
   ...
   i2c->CR1.value = 0x00000000;
   i2c->CR2.value = 0x00000000;
   i2c->OAR1.value = 0x00000000;
   i2c->OAR2 = 0x00000000;
   i2c->CCR.value = 0x00000000;
   i2c->TRISE.value = 0x00000002;
   i2c->FLTR = 0x00000000;
   
   // 4. Disable peripheral clock
   i2c_disable_clock(i2c);
